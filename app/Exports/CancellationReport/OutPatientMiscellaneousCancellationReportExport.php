...
class OutPatientMiscellaneousCancellationReportExport implements FromCollection, WithHeadings, WithMapping
{
    /**
    ...
    public function map($cancellation_report): array
    {
        ...
        foreach ($this->selected_export_fields as $field) {
            switch ($field) {
                ...
                case 'cancel_date':
                    $row[] = date('Y-m-d', strtotime($cancellation_report?->updated_at));
                    break;

                case 'service_date':
                    $row[] = date('Y-m-d', strtotime($cancellation_report?->created_at));
                    break;

                case 'umr':
                    $row[] = $cancellation_report?->opdBilling?->patient?->registration_no;
                    break;

                case 'patient_name':
                    $row[] = $cancellation_report?->opdBilling?->patient?->name;
                    break;
