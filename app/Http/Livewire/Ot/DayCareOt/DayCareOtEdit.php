...
class DayCareOtEdit extends Component
{
    use OtAttendantUpdate;

    public $day_care_id, $day_care_no, $day_care_date, $type = "scheduled-patients", $status = "Not Approved";
    ...
    public function mount($id)
    {
        ...
        $ot_day_care = OtDayCare::with('attendants')->find($id);
        if ($ot_day_care) {
            $this->day_care_id = $ot_day_care->id;
            $this->day_care_no = $ot_day_care->code;
            $this->day_care_date = date("Y-m-d H:i", strtotime($ot_day_care->created_at));
            $this->status = "Approved";
            $this->ot_booking_id = $ot_day_care->ot_booking_id;

            $this->ipd_id = $ot_day_care->ipd_id;
            $this->patient_id = $ot_day_care->patient_id;
            $this->umr = $ot_day_care?->patient?->registration_no;
            ...
            $this->type = $ot_day_care->type;
            $this->service_id = $ot_day_care->service_id;
            $this->serviceChanged();
            $this->surgery_type_id = $ot_day_care->surgery_type_id;
            $this->surgeryTypeChanged();
            $this->ot_id = $ot_day_care->ot_id;
            $this->otChanged();
            $this->surgery_date = date("Y-m-d", strtotime($ot_day_care->surgery_date));
            $this->from_time = date("H:i", strtotime($ot_day_care->from_time));
            $this->to_time = date("H:i", strtotime($ot_day_care->to_time));
            $this->duration = $ot_day_care->duration;
            $this->doctor_id = $ot_day_care->doctor_id;
            $this->doctorChanged();
            $this->anesthesia_type_id = $ot_day_care->anesthesia_type_id;
            $this->diagnosis = $ot_day_care->diagnosis;
            $this->remarks = $ot_day_care->remarks;

            $this->ot_booking_no = $ot_day_care?->ot_booking?->code;
            $this->ot_booking_date = date("Y-m-d H:i", strtotime($ot_day_care?->ot_booking?->created_at));

            $this->ot_day_care_attendant_init($ot_day_care);
        }
    }
    ...
    public function save()
    {
        ...
        $ot_day_care = OtDayCare::find($this->day_care_id);
        if ($ot_day_care) {
            $ot_day_care->update([
            ...
            OtDayCareConsultant::where('ot_day_care_id', $this->day_care_id)->delete();
    ...
    {
        return view('livewire.ot.day-care-ot.day-care-ot-edit')->extends('layouts.admin')->section('content');
    }
}
