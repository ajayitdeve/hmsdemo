...
class InternalTransferGin extends Component
{
    ...
    public function mount()
    {
        ...
        //getting stock_point of loggedin use
        $stockPoint = StockPoint::find(session()->get("stock_point_id"));
        $this->loginUserStockPoint = $stockPoint;

        $this->mrqs = Mrq::where('stock_point_to_id', $this->loginUserStockPoint?->id)->orderBy('id', 'DESC')->get();
        // dd($this->mrqs);
    ...
    {
        //except  central pharmacy .. OP/OT/EMG
        $gins = Gin::when($this->loginUserStockPoint, function ($query) {
            return $query->where('stock_point_from_id', $this->loginUserStockPoint?->id);
        })->get();

        return view('livewire.pharmacy-internal-transfer.internal-transfer-gin', ['gins' => $gins])->extends('layouts.admin')->section('content');
    }
}
