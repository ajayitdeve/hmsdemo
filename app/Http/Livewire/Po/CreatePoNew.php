...
class CreatePoNew extends Component
{
    ...
    public function mount($purchase_indent_id)
    {
        ...
        //New Code getting all items in indent_items table where $purchase_indet_id matches
        $this->itemList = IndentItem::where('purchase_indent_id', $purchase_indent_id)->get();

        foreach ($this->itemList as $item) {
            $temp = [];
            $temp['item_id'] = $item->item_id;
            $temp['item_description'] = $item->item->description;
            $temp['quantity'] = $item->quantity;
            $temp['unitrate'] = 0;
            $temp['unitsalerate'] = 0;
            $temp['bonus'] = 0;
            $temp['igst'] = $item->item->igst;
            $temp['discount_percent'] = 0;
            $temp['discounted_amount'] = 0;
            $temp['row_total'] = 0;
            array_push($this->itemListArr, $temp);
        }
        $this->inputs = $this->itemListArr;

        $this->item_id = array_map(function ($elem) {
            return $elem['item_id'];
        }, $this->itemListArr);

        $this->item_description = array_map(function ($elem) {
            return $elem['item_description'];
        }, $this->itemListArr);

        $this->quantity = array_map(function ($elem) {
            return $elem['quantity'];
        }, $this->itemListArr);

        $this->unitrate = array_map(function ($elem) {
            return $elem['unitrate'];
        }, $this->itemListArr);

        $this->unitsalerate = array_map(function ($elem) {
            return $elem['unitsalerate'];
        }, $this->itemListArr);

        $this->bonus = array_map(function ($elem) {
            return $elem['bonus'];
        }, $this->itemListArr);
        $this->igst = array_map(function ($elem) {
            return $elem['igst'];
        }, $this->itemListArr);

        $this->discount_percent = array_map(function ($elem) {
            return $elem['discount_percent'];
        }, $this->itemListArr);

        $this->discounted_amount = array_map(function ($elem) {
            return $elem['discounted_amount'];
        }, $this->itemListArr);

        $this->row_total = array_map(function ($elem) {
            return $elem['row_total'];
        }, $this->itemListArr);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.po.create-po-new')->extends('layouts.admin')->section('content');
    }
    ...
    public function store()
    {
        ...
        foreach ((array) $this->item_id as $key => $value) {
            \App\Models\PurchaseOrderItem::create([
                ...
                'unitrate' => $this->unitrate[$key],
                'unitsalerate' => $this->unitsalerate[$key],
                'bonus' => $this->bonus[$key] != null ? $this->bonus[$key] : 0,
                'discount_percent' => $this->discount_percent[$key]
            ]);
        }
        //change the status in purchase_indetns table 1 ; means po created
        $currentPurchaseIndent = PurchaseIndent::find($this->purchase_indent_id)->update([
            'status' => 1
        ]);
