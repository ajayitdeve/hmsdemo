                    ...
                    <tr>
                        @foreach ($selected_export_fields as $field)
                            @switch($field)
                                ...
                                @case('cancel_date')
                                    <td>{{ date('Y-m-d', strtotime($cancellation_report?->updated_at)) }}</td>
                                @break

                                @case('service_date')
                                    <td>{{ date('Y-m-d', strtotime($cancellation_report?->created_at)) }}</td>
                                @break

                                @case('umr')
                                    <td>
                                        {{ $cancellation_report?->opdBilling?->patient?->registration_no }}
                                    </td>
                                @break

                                @case('patient_name')
                                    <td>
                                        {{ $cancellation_report?->opdBilling?->patient?->name }}
                                    </td>
                                @break
