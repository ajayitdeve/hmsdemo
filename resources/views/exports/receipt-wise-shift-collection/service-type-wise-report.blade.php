    ...
    <table border="2" width="100%">
        ...
        <tbody>
            @foreach ($receipt_wise_shift_collections as $receipt_wise_shift_collection)
                <tr>
                    <td>
                        {{ $service_types["$receipt_wise_shift_collection->type"] }}
                    </td>
                    <td>{{ $receipt_wise_shift_collection->total_qty }}</td>
                    <td>{{ $receipt_wise_shift_collection->total_gross }}</td>
                    <td>{{ $receipt_wise_shift_collection->total_gross }}</td>
                    <td>{{ $receipt_wise_shift_collection->total_discount }}</td>
                    <td>{{ $receipt_wise_shift_collection->total_paid }}</td>
                    <td>{{ $receipt_wise_shift_collection->total_due }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
