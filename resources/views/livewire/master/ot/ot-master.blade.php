    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">OT</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active">OT</li>
                    </ul>
                </div>
                        ...
                        <tbody>
                            @foreach ($ot_list as $ot)
                                <tr>
                                    <td>{{ $ot->name }}</td>
                                    <td>{{ $ot->code }}</td>
                                    <td>{{ $ot?->ot_type?->code }}</td>
                                    <td>{{ $ot?->ot_type?->name }}</td>
                                    <td>{{ $ot?->cost_center?->name }}</td>
                                    <td>{{ $ot->is_active ? 'Y' : 'N' }}</td>
                                    <td>{{ $ot?->created_by?->name }}</td>
                                    <td>{{ $ot->created_at }}</td>
                                    <td class="text-right">
                                        <div class="dropdown dropdown-action">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <button wire:click="edit({{ $ot->id }})" class="dropdown-item"
                                                    href="#" data-toggle="modal" data-target="#edit"><i
                                                        class="fa fa-pencil m-r-5"></i> Edit</button>
                                                <button wire:click="delete({{ $ot->id }})" class="dropdown-item"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add OT</h5>
                ...
                <div class="modal-body">
                    <form wire:submit.prevent='save'>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" wire:model="ot_type_id">
                                <option value="">Select OT Type</option>
                                @foreach ($ot_types as $ot_type)
                                    <option value="{{ $ot_type->id }}">{{ $ot_type->name }}</option>
                                @endforeach
                            </select>
                            @error('ot_type_id')
                                <span class="text-danger">{{ $message }}</span>
