     ...
     <div class="modal-dialog modal-dialog-centered" role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title">Add Item Specialization </h5>
             ...
             <div class="modal-body">
                 <form wire:submit.prevent='save'>
                     <div class="form-group">
                         <label>Name</label>
                         <input class="form-control" type="text" wire:model='name' placeholder="Specialization Name">
                         @error('name')
                 ...
                 <form wire:submit.prevent='update'>
                         ...
                         <select wire:model='cost_center_id' class="form-control">
                             ...
                             @foreach ($costcenters as $costcenter)
                                 <option value="{{ $costcenter->id }}"
                                     {{ $costcenter->id == $cost_center_id ? 'selected' : null }}>
                                     {{ $costcenter->name }} - {{ $costcenter->code }}</option>
                             @endforeach
                         </select>
                         @error('cost_center_id')
                             <span class="text-danger">{{ $message }}</span>
                     ...
                     <div class="form-group">
                         ...
                         <select wire:model='type_id' class="form-control">
                             ...
                             @foreach ($types as $type)
                                 <option value="{{ $type->id }}" {{ $type->id == $type_id ? 'selected' : null }}>
                                     {{ $type->name }} </option>
                             @endforeach
                         </select>
                         @error('type_id')
